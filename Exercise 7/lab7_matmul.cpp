...
void reference_matmul_bf16(
    ...
    TT_FATAL(A.size() == M * K, "A must be M*K");
    TT_FATAL(B.size() == K * N, "B must be K*N");
    TT_FATAL(C.size() == M * N, "C must be M*N");
...
void eltwise_add_tensix(
    ...
    const uint32_t a_elements = M * K;
    const uint32_t b_elements = K * N;
    const uint32_t total_elements = M * N;
    TT_FATAL(a.size() == a_elements, "A must have size M * K");
    TT_FATAL(b.size() == b_elements, "B must have size K * N");
    TT_FATAL(output.size() == total_elements, "Output must have size M * N");

    // Tile assumptions for matmul
    TT_FATAL(TILE_HEIGHT == TILE_WIDTH, "Tiles must be square");
   ...
   TensorLayout tile_layout(
    DataType::BFLOAT16,
    PageConfig(Layout::TILE),
    MemoryConfig(BufferType::DRAM)
);

// A: M x K
TensorSpec a_spec(Shape({M, K}), tile_layout);
// B: K x N
TensorSpec b_spec(Shape({K, N}), tile_layout);
// C: M x N
TensorSpec c_spec(Shape({M, N}), tile_layout);

Tensor src0_tensor = Tensor::from_vector<bfloat16>(a, a_spec, prog_state.mesh_device.get());
Tensor src1_tensor = Tensor::from_vector<bfloat16>(b, b_spec, prog_state.mesh_device.get());
Tensor dst_tensor  = create_device_tensor(c_spec, prog_state.mesh_device.get());
    ...
    KernelHandle reader_id = tt_metal::CreateKernel(
        prog_state.program,
        OVERRIDE_KERNEL_PREFIX "ttnn/examples/lab7_matmul/kernels/dataflow/read_matmul_tiles.cpp",
        prog_state.core,
    ...
    KernelHandle writer_id = tt_metal::CreateKernel(
        prog_state.program,
        OVERRIDE_KERNEL_PREFIX "ttnn/examples/lab7_matmul/kernels/dataflow/write_matmul_tiles.cpp",
        prog_state.core,
        tt_metal::DataMovementConfig{
    ...
    std::vector<uint32_t> compute_compile_time_args = {Mt, Nt, Kt};

    tt_metal::CreateKernel(
        prog_state.program,
        OVERRIDE_KERNEL_PREFIX "ttnn/examples/lab7_matmul/kernels/compute/matmul_tiles.cpp",
        prog_state.core,
    ...
    tt_metal::SetRuntimeArgs(prog_state.program, reader_id, prog_state.core, {src0_addr, src1_addr, Mt, Nt, Kt});
    tt_metal::SetRuntimeArgs(prog_state.program, writer_id, prog_state.core, {dst_addr, Mt, Nt});
...
int main() {
    ...
    try {
        ...
        reference_matmul_bf16(a_vec, b_vec, reference_result, M, K, N);
        ...
        eltwise_add_tensix(a_vec, b_vec, result_vec, M, K, N, prog_state);
        auto d1 = std::chrono::high_resolution_clock::now();
        double device_ms = std::chrono::duration<double, std::milli>(d1 - d0).count();
        ...
        for (size_t i = 0; i < result_vec.size(); ++i) {
            ...
            max_rel_err  = std::max(max_rel_err, relative_error);
        ...
        log_info(tt::LogAlways, "Reference time: {} ms", ref_ms);
        log_info(tt::LogAlways, "Device time: {} ms", device_ms);
        log_info(tt::LogAlways, "Max abs diff: {}", max_abs_diff);
        log_info(tt::LogAlways, "Max rel err: {}", max_rel_err);
